...
public class ProductControllerTests {
    ...
    public void  get_all_products_controller_should_return404_when_productList_isEmpty() throws Exception {
        ...
        //when
        when(productService.getAllProduct()).thenReturn(emptyProducts);
    ...
    public void get_all_product_by_category_controller_should_return200_when_validRequest() throws Exception {
        //when
        when(productService.getAllProductByCategoryId(PRODUCT_CATEGORY_ID)).thenReturn(products);

        //then - Truyền param categoryId thay vì category
        mockMvc.perform(get("/products").param("categoryId", String.valueOf(PRODUCT_CATEGORY_ID)))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
                .andExpect(jsonPath("$[0].id").value(PRODUCT_ID))
                .andExpect(jsonPath("$[0].categoryId").value(PRODUCT_CATEGORY_ID.intValue()));

        verify(productService, times(1)).getAllProductByCategoryId(anyLong());
        verifyNoMoreInteractions(productService);
    }
    ...
    public void get_all_product_by_category_controller_should_return404_when_productList_isEmpty() throws Exception {
        ...
        //when
        when(productService.getAllProductByCategoryId(PRODUCT_CATEGORY_ID)).thenReturn(emptyProducts);
        
        //then - Truyền param categoryId
        mockMvc.perform(get("/products").param("categoryId", String.valueOf(PRODUCT_CATEGORY_ID)))
                .andExpect(status().isNotFound())
                .andExpect(content().contentType(MediaType.APPLICATION_PROBLEM_JSON_UTF8));
    ...
    public void get_one_product_by_id_controller_should_return200_when_validRequest() throws Exception {
        ...
        mockMvc.perform(get("/products/{id}", PRODUCT_ID))
                ...
                .andExpect(jsonPath("$.categoryId").value(PRODUCT_CATEGORY_ID.intValue()));
